...
IS
BEGIN
	UPDATE USUARIO SET USUARIO=p_usuario, PASSWORD=p_password, ESTADO_USUARIO=p_estado_usuario, ROL_ID_ROL=p_rol_id_rol WHERE ID_USUARIO=p_id_usuario;
	COMMIT;
END;
/
...
CREATE OR REPLACE PROCEDURE UPDATE_TERMINAL(
	p_id_terminal IN TERMINAL.ID_TERMINAL%TYPE,
...
IS
BEGIN
	UPDATE TERMINAL SET AGENCIA_ID_AGENCIA=p_agencia_id_agencia, USUARIO_ID_USUARIO=p_usuario_id_usuario, ESTADO_TERMINAL=p_estado_terminal WHERE ID_TERMINAL = p_id_terminal;
	COMMIT;
END;
/

-- ROL PERMISO
CREATE OR REPLACE PROCEDURE UPDATE_ROL_PERMISO(
...
IS
BEGIN
	UPDATE ROL_PERMISO SET ROL_ID_ROL=p_rol_id_rol, PERMISO_ID_PERMISO=p_permiso_id_permiso, ESTADO_ROL_PERMISO=p_estado_rol_permiso WHERE ROL_ID_ROL=p_rol_id_rol AND PERMISO_ID_PERMISO=p_permiso_id_permiso;
	COMMIT;
END;
/